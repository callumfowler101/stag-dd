...
import { CLASSES, TITLES, STAT_ORDER, STAT_COLORS } from '../stores/game_data.js'
        ...
        return (
          <span key={i} style={{ display: 'flex', alignItems: 'center' }}>
            <span className={styles.stepNode}>
              ...
              <span className={`${styles.stepLbl}${active ? ' ' + styles.stepLblActive : ''}`}>{lbl}</span>
            </span>
  ...
  return (
    <div className={styles.mstat}>
      ...
      <div className={styles.mstatBg}>
        <div className={styles.mstatFill} style={{ width: `${w}%`, background: STAT_COLORS[stat] }} />
      </div>
    ...
    >
      ...
      <div className={styles.clsBody}>
        ...
        <div className={styles.miniStats}>
          {cls.top.map((s) => <MiniStat key={s} stat={s} value={cls.stats[s]} />)}
        </div>
      </div>
    </div>
  );
...
function StatBar({ stat, value, delay = 0 }) {
  const [w, setW] = useState(0);
  useEffect(() => { const t = setTimeout(() => setW(value), 220 + delay); return () => clearTimeout(t); }, [value, delay]);
  return (
  ...
  return (
    <div className={`panel fade-in ${styles.wizardPanel}`}>
      <div className="panel-body">
        ...
        <div className="sec-sub">Every hero begins with a choice. Who will you be?</div>
        <div className={styles.classGrid}>
          {CLASSES.map((cls) => (
            <ClassCard key={cls.id} cls={cls} selected={selected?.id === cls.id} onSelect={onSelect} />
          ))}
        </div>
        <div className="btn-row">
          <button className="rs-btn rs-btn-primary" disabled={!selected} onClick={onNext}>
            Continue →
          </button>
        </div>
      </div>
...
function Step2({ selectedClass, onBack, onNext }) {
  ...
  const submit = () => { if (name.trim()) onNext({ name: name.trim(), title, fullName: full }); };
  return (
    <div className={`panel fade-in ${styles.wizardPanel}`}>
      <div className="panel-body">
        ...
        <div className={styles.nameForm}>
          ...
          <div>
            ...
            <div className={styles.namePreview}>
              {full
                ? full
                : <span className={styles.namePreviewPh}>Your name will appear here…</span>
              }
            </div>
          </div>
        </div>
        <div className="btn-row">
          <button className="rs-btn rs-btn-ghost" onClick={onBack}>← Back</button>
          <button className="rs-btn rs-btn-primary" disabled={!name.trim()} onClick={submit}>
            Forge Your Legend →
          </button>
        </div>
      </div>
  ...
  return (
    <div className={`panel fade-in ${styles.wizardPanel}`}>
      <div className="panel-body">
        ...
        <div className="sec-sub">Commit your soul to the dungeon, {character.name}.</div>
        <div className={styles.previewWrap}>
          ...
          <div className="stat-section">
            {STAT_ORDER.map((s, i) => (
              <StatBar key={s} stat={s} value={selectedClass.stats[s]} delay={i * 110} />
            ))}
          </div>
        </div>
        <div className="btn-row">
  ...
  return (
    <div className={`panel fade-in ${styles.wizardPanel}`}>
      <div className={`panel-body ${styles.successBody}`}>
        ...
        <div className="sec-title" style={{ textAlign: 'center' }}>The Adventure Begins</div>
        <p style={{ color: 'var(--text-muted)', fontSize: 14, marginTop: 8, fontStyle: 'italic' }}>
          {character.fullName}, {selectedClass.name} of legend, has joined the party.
        </p>
        <div className={styles.uuidBox}>
          <div className={styles.uuidLbl}>YOUR HERO ID</div>
          <div className={styles.uuidVal}>#{uuid}</div>
        </div>
        <p className={styles.successNote}>Keep this code to access your character sheet.</p>
        <div className="btn-row" style={{ justifyContent: 'center', marginTop: 24 }}>
          <button className="rs-btn rs-btn-ghost" onClick={() => window.location.reload()}>
            Create Another Hero
          </button>
        </div>
      </div>
      ...
      <div className={styles.root}>
        ...
        {step === 2 && (
          <Step2
            ...
            onBack={() => setStep(1)}
            onNext={(char) => { setCharacter(char); setStep(3); }}
          />
        )}
        {step === 3 && (
          <Step3
